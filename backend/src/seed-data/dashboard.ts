// Dashboard fixtures + derivations ported verbatim from the Origami v4 design prototype.
// These mirror the prototype's client-side computed data so the dashboard renders
// identically, including role variants and contract-term-based invoice classification.
...
export const FINANCE: FinanceRow[] = [
...
interface Terms { net: number; ret: number; billing: string; release: string }

const CONTRACT_TERMS: Record<string, Terms> = {
...
export const INVOICES: Record<ViewMode, RawInvoice[]> = {
  internal: [
...
export function enrichInvoices(list: RawInvoice[]): EnrichedInvoice[] {
...
export const REVENUE_BASE: Record<ViewMode, { month: string }[]> = {
...
export interface FunnelRow { label: string; n: string; v: number }
export const FUNNEL: Record<ViewMode, FunnelRow[]> = {
  client: [
...
export interface TeamRow { name: string; role: string; kind: string; tasks: number; done: number; av: string }
...
export interface Deadline { task: string; project: string; due: string; domain: string; owner: string; past: boolean }
...
export const money = (n: number) =>
  n >= 1000000 ? '$' + (n / 1000000).toFixed(2) + 'M' : n >= 1000 ? '$' + Math.round(n / 1000) + 'K' : '$' + n;
