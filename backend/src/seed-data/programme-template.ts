// The programme every new project starts from.
//
// This file is only the *default*. The live template is stored in settings
// under `programme.template` and edited in the app, so the office can add,
// rename, reorder and delete phases and tasks without a deploy. This default is
// what a database gets before anyone has edited anything.
...
/** Teams offered when editing a task, so the labels stay consistent. */
...
/** Tags a template task can carry. */
export const TEMPLATE_LABELS = ['Deliverable', 'Approval', 'Auto', 'Milestone'];

/** Reject anything that is not a usable template, so a bad save cannot brick seeding. */
export function parseProgramme(raw: string | null | undefined): TemplatePhase[] | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed) || !parsed.length) return null;
    const phases = parsed
      .filter((p: any) => p && typeof p.key === 'string' && typeof p.name === 'string')
      .map((p: any, i: number) => ({
        key: String(p.key),
        name: String(p.name),
