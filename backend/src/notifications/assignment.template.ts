...
export interface AssignmentEmailInput {
  ...
  settingsUrl: string;
  /** For people following the task: why this mail (added / commented / done), instead of "assigned you". */
  follow?: { kind: 'added' | 'comment' | 'done'; comment?: string };
}
...
export function assignmentEmail(input: AssignmentEmailInput) {
  ...
  const who = `<strong>${escapeHtml(input.assignerName)}</strong>`;
  ...
  const quote = input.follow?.kind === 'comment' && input.follow.comment
    ? `<p style="margin:-8px 0 18px;padding:10px 14px;border-left:3px solid #2F7D4A;background:#F4F7F4;font-size:13.5px;line-height:1.6;color:#0B1A12;white-space:pre-wrap;">${escapeHtml(input.follow.comment.length > 600 ? input.follow.comment.slice(0, 600) + '…' : input.follow.comment)}</p>`
    : '';
  const body = `
    <p style="margin:0 0 18px;font-size:14px;line-height:1.65;color:#43514D;">
      Hi ${escapeHtml(first)} — ${why}
    </p>${quote}
