// Branded HTML for the transactional emails the platform sends.
// Colours mirror the Origami UI (deep green / clay accent on a warm paper ground).

const shell = (title: string, intro: string, cta: { label: string; url: string }, footer: string) => `
<!doctype html>
<html>
...
export function inviteEmail(opts: { name: string; url: string; roleName: string; expiresInDays: number }) {
  const first = opts.name.split(/\s+/)[0] || 'there';
  return {
    subject: 'Set up your Origami account',
    html: shell(
      `Welcome, ${escapeHtml(first)}`,
      `An account has been created for you on the Origami Design + Build platform as <strong>${escapeHtml(opts.roleName)}</strong>. Choose a password to activate it — after that you can sign in with your email and password, or with your Google account.`,
      { label: 'Create your password', url: opts.url },
      `This link expires in ${opts.expiresInDays} days. If you weren't expecting this invitation you can safely ignore this email.`,
    ),
  };
}

export function resetEmail(opts: { name: string; url: string; expiresInHours: number }) {
  const first = opts.name.split(/\s+/)[0] || 'there';
  return {
    subject: 'Reset your Origami password',
    html: shell(
      `Password reset for ${escapeHtml(first)}`,
      'We received a request to reset the password on your Origami account. Choose a new one using the button below.',
      { label: 'Choose a new password', url: opts.url },
      `This link expires in ${opts.expiresInHours} hours. If you didn't request a reset, nothing has changed — you can ignore this email.`,
    ),
  };
}
