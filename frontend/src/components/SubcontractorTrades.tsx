...
export function SampleDataPanel({ canManage, onChanged }: { canManage: boolean; onChanged: () => Promise<unknown> | void }) {
  const { toast } = useApp();
  const [status, setStatus] = useState<{ loaded: boolean; employees: number; contractors: number; payrollRuns?: number; timesheets?: number } | null>(null);
  ...
  return (
    <div style={{ ...card, padding: '18px 20px', maxWidth: 760 }}>
      ...
      <div style={{ fontSize: 12.5, color: MUTED, lineHeight: 1.7, margin: '6px 0 14px' }}>
        ...
        payroll (last month finalized and paid by direct deposit, this month as a draft), office staff timesheets (approved, sent back, awaiting review), shift rosters, company assets, crew housing and transport routes.
      ...
      {status === null ? <div style={{ fontSize: 12.5, color: MUTED }}>Checking…</div> : (
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          {status.loaded
            ? <Badge tone="green">Loaded · {status.employees} people, {status.contractors} contractors, {status.payrollRuns || 0} payroll runs, {status.timesheets || 0} timesheets</Badge>
          ...
          {canManage && status.loaded && (!status.payrollRuns || !status.timesheets) && (
            <div onClick={busy ? undefined : () => act(() => api.sampleData.loadPayroll(), 'Sample payroll and timesheets added')} style={btn(true, busy)}>{busy ? 'Adding…' : 'Add sample payroll & timesheets'}</div>
          )}
